...
const tryGetTsCompilerOptionsFromTsConfigJsonFile = (configFilePath?: string) => {
  if (!configFilePath) {
    const currentDir = process.cwd();
    configFilePath = ts.findConfigFile(currentDir, ts.sys.fileExists, "tsconfig.json");
  }
  if (configFilePath) {
    const {
      config
    ...
    if (config.compilerOptions) {
      const {
        options
      } = ts.convertCompilerOptionsFromJson(config?.compilerOptions, ".", configFilePath);
      return options;
    }
  }
};
const supportExtensions = [".ts", ".tsx", ".d.ts", ".js", ".jsx", ".cts", ".d.cts", ".cjs", ".mts", ".d.mts", ".mjs"];
export const TsToDtsTransformPlugin = (options?: {
  ...
  return defineTransformPlugin({
    ...
    beforeTransform: async (input, finalUserConfig) => {
      const tsCompilerOptionsFromFile = tryGetTsCompilerOptionsFromTsConfigJsonFile(options?.configFile);
      const compilerOptions = {
        ...tsCompilerOptionsFromFile,
        ...options?.compilerOptions
      };
      const files = genDts({
        fileNames: input.filter(e => {
